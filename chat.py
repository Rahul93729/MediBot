...
class ChatApplication(tk.Tk):
    ...
    def _setup_ui(self):
        ...
        # Message entry box
        self.entry = tk.Entry(bottom_label, bg="black", fg="white", font="Helvetica 14")  # Changing entry box color to black
        self.entry.place(relwidth=0.74, relheight=0.08, rely=0.008, relx=0.015)
        self.entry.focus()
        self.entry.bind("<Return>", self.send)
        ...
        send_button.place(relx=0.77, rely=0.008, relheight=0.08, relwidth=0.26)
    ...
    def send(self, event=None):
        try:
            user_input = self.entry.get()
            if user_input.lower() == 'quit':
                self.output_area.insert(tk.END, "MediBot: Take care. See you soon.\n")
                self.output_area.yview(tk.END)
            else:
                result = self.model.predict(keras.preprocessing.sequence.pad_sequences(self.tokenizer.texts_to_sequences([user_input]), truncating='post', maxlen=self.max_len))
                tag = self.lbl_encoder.inverse_transform([np.argmax(result)])
                for i in data['intents']:
                    if i['tag'] == tag:
                        ...
                        self.output_area.insert(tk.END, "You: " + user_input + "\n", 'user_msg')
                        self.output_area.insert(tk.END, "\n", 'spacing')
                        self.output_area.insert(tk.END, "MediBot: " + response + "\n", 'bot_msg')
                        self.output_area.insert(tk.END, "\n\n", 'spacing')
                        self.output_area.tag_config('user_msg', justify='left')  # Align user messages to the left
                        self.output_area.tag_config('bot_msg', justify='right')  # Align bot responses to the right
                        self.output_area.tag_config('spacing', spacing2=10)  # Add spacing between messages
                        self.output_area.configure(state="disabled")
                        self.output_area.yview(tk.END)
            self.entry.delete(0, tk.END)
